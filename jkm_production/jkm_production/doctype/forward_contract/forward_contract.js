...
cur_frm.set_query("document", "forward_contract_underlying", function(doc, cdt, cdn){
...
frappe.ui.form.on("Forward Contract", {
	...
	validate:function(frm){
		if (frm.doc.total_underlying>frm.doc.amount){
			frappe.throw("Amount underlying should not be greater than forward Contract amount.")
		}
	},
	hedge: function(frm){
		var df = frappe.meta.get_docfield("Forward Contract Underlying","link_to", frm.doc.name);
		...
		frm.refresh_field("forward_contract_underlying");
	},
	...
	maturity_to: function(frm){
		...
		frm.trigger("forward_contract_calculation");
	},
	...
	forward_contract_calculation: function(frm){
...
frappe.ui.form.on("Forward Contract Underlying", {
	...
	amount_covered: function(frm, cdt, cdn){
		let total = 0.0;
		frm.doc.forward_contract_underlying.forEach(function(d) {
...
frappe.ui.form.on("Forward Contract Cancellation", {
